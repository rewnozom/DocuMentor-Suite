...
class CompatibilityTrainingGenerator:
    ...
    def _process_file(self, file_path: Path):
        """Processar en fil och extraherar kompatibilitetsrelationer"""
        ...
        if relations:
            ...
            # Generera träningsexempel för varje relation
            for relation in relations:
                try:
                    # Använd formatter för att skapa träningsexempel
                    examples = self.formatter.format_for_training(relation)
                    
                    if not examples:  # Om inga exempel genererades
                        continue
                        
                    # Lägg till i träningsdata
                    ...
                    for example in examples:
                        if not isinstance(example, TrainingExample):
                            logger.error(f"Ogiltig exempeltyp: {type(example)}")
                            continue
                            
                        cmd = example.user_input.split()[0]  # Ex: '-c'
    ...
    def _save_training_data(self):
        ...
        try:
            # Verifiera att vi har korrekta objekt
            valid_examples = [
            ...
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(
                    [ex.__dict__ for ex in valid_examples],
                    f,
                    ensure_ascii=False,
                    indent=2
                )
