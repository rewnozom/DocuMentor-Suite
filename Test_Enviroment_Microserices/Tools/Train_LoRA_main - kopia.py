...
Denna pipeline:
  ...
  • Laddar träningsdata från en Parquet-fil (ex. "combined_dedup.parquet")
  ...
  • Döper om den kompilerade mappen genom att lägga till suffixet " (Copiax)".
  • Laddar upp hela modellen (LoRA-adapter, kompilerad version, GGUF-versioner, m.m.)
    till Hugging Face Hub.
  
Alla inställningar är nu inbyggda direkt i denna kod.
"""
...
# -------------------------
# [Configuration] - Inline inställningars
# -------------------------
CONFIG = {
    "parquet_dataset_path": "./combined_dedup_final.parquet", #combined_dedup / combined_dedup_final
    "processed_dataset_path": "processed_dataset2",
    "output_dir": "./output2",
    ...
    "learning_rate": 4e-4,  # Se till att detta är ett flyttal, ej en sträng
    "warmup_ratio": 0.2,
...
# -------------------------
# [Rensa tidigare körningsdata]
# -------------------------
# Ta bort tidigare processad dataset om mappen finns
if os.path.exists(CONFIG["processed_dataset_path"]):
    shutil.rmtree(CONFIG["processed_dataset_path"])
    print(f"Rensade tidigare processad dataset: {CONFIG['processed_dataset_path']}")

# Ta bort tidigare output-katalog (inkluderar checkpoints mm.) om mappen finns
if os.path.exists(CONFIG["output_dir"]):
    shutil.rmtree(CONFIG["output_dir"])
    print(f"Rensade tidigare output-katalog: {CONFIG['output_dir']}")
...
if os.path.exists(PARQUET_PATH):
    ...
    logger.error(f"🚨 Parquet-fil hittades inte: {PARQUET_PATH}")
...
num_epochs = CONFIG.get("num_epochs", 55)
...
# Döpa om den kompilerade mappen så att den inkluderar " (Copiax)" i namnet
new_compiled_dir = compiled_dir.parent / (compiled_dir.name + " (Copiax)")
try:
    if new_compiled_dir.exists():
        shutil.rmtree(new_compiled_dir)
    shutil.move(str(compiled_dir), str(new_compiled_dir))
    logger.info(f"✅ Kompilerad mapp döpt om till: {new_compiled_dir}")
except Exception as e:
    logger.error(f"Fel vid omdöpning av kompilerad mapp: {e}")
...
hf_username = "rewnozom"  # Ditt Hugging Face-användarnamn
model_repo = CONFIG.get("output_model_repo", "rewnozom/Copiax_V1a.1")
REPO_ID = model_repo  # Repo-ID innehåller redan "Copiax" i namnet
...
try:
    shutil.copytree(new_compiled_dir, formats["compiled"], dirs_exist_ok=True)
...
Detta är en finetunad LoRA-modell för DocuMentor-Suite (Copiax).
...
### Primärt Kommando
- `-c` - Används för att söka kompatibilitetsinformation
  ```bash
  # Exempel på användning:
  -c 50093073             # Sök med artikelnummer
  -c "ASSA ABLOY Trycke"  # Sök med produktnamn
  ```
  
Output: Strukturerad markdown med fullständig produktinformation inklusive:
- Produktbeskrivning
- Tekniska specifikationer
- Artikelinformation
- Förpackningsinformation

## Rekommenderad GGUF Version
- För bästa prestanda/storleksförhållande: `q4_k_m`
- För högsta kvalitet: `f16`
- För minimal storlek: `q2_k`

## Licens och Användning
Denna modell är specifikt gjort för Copiax.

## Kontakt
För frågor, kontakta [Tobiasraanaes.se] - tobias.raana@outlook.com
"""
